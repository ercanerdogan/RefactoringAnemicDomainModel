...
public class CustomerName : ValueObject<CustomerName>
{
    ...
    public static Result<CustomerName> Create(string customerName)
    {
        ...
        if (customerName.Length == 0) return Result.Fail<CustomerName>("Customer name should not be empty");

        if (customerName.Length > 100) return Result.Fail<CustomerName>("Customer name is too long");
